...
public class XAxisRenderer extends AxisRenderer {
    ...
    protected void drawLabels(Canvas c, float pos, PointF anchor) {
        ...
        for (int i = mMinX; i <= mMaxX; i += mXAxis.mAxisLabelModulus) {
            ...
            if (mViewPortHandler.isInBoundsX(position[0])) {
                ...
                if (MpChartSetdate.XlineStype== 20170315) {
                    if (!oldtime.equals( label)) {
                        oldtime =  label;
                    } else {
                        label= "";
                    }
    ...
    protected void drawLabel(Canvas c, String label, int xIndex, float x, float y, PointF anchor, float angleDegrees) {
        ...
        Utils.drawXAxisValue(c, formattedLabel, x, y, mAxisLabelPaint, anchor, angleDegrees, mViewPortHandler.contentBottom());
    }
