...
import com.seeweekend.code.charting.components.YAxis.YAxisLabelPosition;
...
public class YAxisRenderer extends AxisRenderer {
    ...
    protected void drawYLabels(Canvas c, float fixedPosition, float[] positions, float offset) {
        ...
        String units="";
        try {
            List<xyvals> ylist = mYAxis.getMyYvals();
            if (ylist != null && ylist.size() > 0) {//在此处修改Y显示，但要和数据一致
                showtype = ylist.get(0).getIndex();
                changnumber = Float.valueOf(ylist.get(0).getIndexsec());
                units=ylist.get(0).getMarksec();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        for (int i = 0; i < mYAxis.mEntryCount; i++) {
            ...
            if (!mYAxis.isDrawTopYLabelEntryEnabled() && i >= mYAxis.mEntryCount - 1)
                return;
            if (showtype == MpChartSetdate.editSleppTYPE) {
                text = "_____";
                c.drawText(text, fixedPosition, positions[i * 2 + 1], mAxisLabelPaint);
            } else if (showtype == MpChartSetdate.YSLEEPTYPE) {
                if (text.trim().equals("0") || text.trim().equals("100")) {
                    text = "_____";
                    c.drawText(text, fixedPosition, positions[i * 2 + 1], mAxisLabelPaint);
                }
            } else {
                if (showtype == MpChartSetdate.tempmax+MpChartSetdate.mpLinechartActivitykey) { //这种情况可以用到所有负数类型传值
                        float airfloat = Float.valueOf(text);
                        text = (int) (airfloat + changnumber) + units;
                    mAxisLabelPaint.setTextSize(15);
                }else if(showtype !=-1&&!units.isEmpty()){
                    mAxisLabelPaint.setTextSize(15);
                    text=text+units;
                }
