package com.seeweekend.code.charting.renderer;
...
public class LineChartRenderer extends LineRadarRenderer {
     ...
     */
    protected Bitmap.Config mBitmapConfig = Bitmap.Config.ALPHA_8;//ARGB_4444  ARGB_8888 RGB_565 ALPHA_8
    ...
    protected List<Path> cubicPaths = new ArrayList<>();
    protected List<Path> cubicFillPaths = new ArrayList<>();
    ...
    protected void drawCubicBezier(Canvas c, ILineDataSet dataSet) {
        ...
        if (size - minx >= 2) {
            ...
            if (dataSet.mpChartSetdate() != null && dataSet.mpChartSetdate().drawCubicBezieType == MpChartSetdate.defaultType) {
                ismyType = true;
            }
            float prevDx = 0f;
            float prevDy = 0f;
            ...
            for (int j = minx + 1, count = Math.min(size, entryCount); j < count; j++) {
                ...
                float x1 = prev.getXIndex() + prevDx;
                float y1 = (prev.getVal() + prevDy) * phaseY;
                float x2 = cur.getXIndex() - curDx;
                float y2 = (cur.getVal() - curDy) * phaseY;
                float x3 = cur.getXIndex();
                float y3 = cur.getVal() * phaseY;
    ...
    public ArrayList<float[]> getGetCirVels() {
        return getCirVels;
    }

    public void setGetCirVels(ArrayList<float[]> getCirVels) {
        this.getCirVels = getCirVels;
    }

    private ArrayList<float[]> getCirVels = new ArrayList<>();
    ...
    protected void drawCircles(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            for (int j = minx,
                 ...
                 j < count;
                 j++) {
                float maxcir = circleRadius;
                if (j == dataSet.mpChartSetdate().mynowindex) {
                    if (dataSet.mpChartSetdate().drawLineType == 2) {

                    }
                    maxcir = circleRadius + addscir;
                ...
                if (j == dataSet.mpChartSetdate().mynowindex) {
                    ...
                    oldindex = j;
                } else if ( dataSet.mpChartSetdate().mynowindex == -1 && dataSet.getYvals() != null) {
                    String myaction = "myaction";
                    if (dataSet.getYvals().get(0) != null) {
                        myaction = dataSet.getYvals().get(0).getMarksec();
                        if (myaction.equals(MotionEvent.ACTION_UP + "") && oldindex != -1) {
                            String[] newstring = (String[]) e.getData();
                            isupshow = true;
                            levehight = 0;
                            if (newstring.length >= 4) {
                                newstring[3] = MotionEvent.ACTION_UP + "";
                            }
                            e.setData(newstring);
                ...
                if (drawTransparentCircleHole) {
                    ...
                    mCirclePathBuffer.addCircle(circlesBuffer[0], circlesBuffer[1],
                            circleHoleRadius,
                            Path.Direction.CCW);
                    if (j ==  dataSet.mpChartSetdate().mynowindex || (isupshow && j == oldindex)) {
                        // Sleepmark.refreshContent();
